...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const submission = await ContactSubmission.create({ name, email, phone, company, service, message });
    return res.status(201).json({ id: submission.id, message: 'Your enquiry has been received.' });
  } catch (error) {
